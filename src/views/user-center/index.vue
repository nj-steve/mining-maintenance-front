...
const profileForm = ref<{ phone: string; email: string; password: string, repeatPassword: string }>({
  phone: '',
  email: '',
  password: '',
  repeatPassword: ''
...
async function handleProfileSubmit() {
  try {
    ...
    if (profileForm.value.password !== profileForm.value.repeatPassword) {
    ...
    console.log("res>>",res,res.response.data.code);
...
<template>
  <div>
    <NCard v-model:value="profileOpen"  style="margin-bottom: 12px;">
      <!-- <NSpaceItem name="profile"> -->
        <template #header>
          <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
            <span>个人信息</span>
            <NButton text type="primary" size="small" @click.stop="handleProfileEdit" v-if="!editingProfile">修改</NButton>
          </div>
        </template>
        <div v-if="!editingProfile">
          <NDescriptions :column="2">
            <NDescriptionsItem>
              <template #label><span style="color: #8086a0;">昵称</span></template>
              <span style="color: #333333;">{{ personalInfo.username }}</span>
            </NDescriptionsItem>
            <NDescriptionsItem>
              <template #label><span style="color: #8086a0;">真实姓名</span></template>
              <span style="color: #333333;">{{ personalInfo.real_name }}</span>
            </NDescriptionsItem>
            <NDescriptionsItem>
              <template #label><span style="color: #8086a0;">联系电话</span></template>
              <span style="color: #333333;">{{ personalInfo.contact_phone || '-' }}</span>
            </NDescriptionsItem>
            <NDescriptionsItem>
              <template #label><span style="color: #8086a0;">邮箱</span></template>
              <span style="color: #333333;">{{ personalInfo.email || '-' }}</span>
            </NDescriptionsItem>
            <NDescriptionsItem>
              <template #label><span style="color: #8086a0;">所属公司</span></template>
              <span style="color: #333333;">{{ personalInfo.company_info?.[0]?.name || '-' }}</span>
            </NDescriptionsItem>
            <NDescriptionsItem>
              <template #label><span style="color: #8086a0;">入职时间</span></template>
              <span style="color: #333333;">{{ personalInfo.start_date || '-' }}</span>
            </NDescriptionsItem>
          </NDescriptions>
        </div>
        <div v-else>
          <NForm :model="profileForm" label-width="100">
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px;">
              ...
              <NFormItem label="密码（不修改可留空）">
                <NInput v-model:value="profileForm.password" type="password" placeholder="请输入密码" />
              </NFormItem>
              <NFormItem label="确认密码（不修改可留空）">
                <NInput v-model:value="profileForm.repeatPassword" type="password" placeholder="请再次输入确认密码" />
              </NFormItem>
            </div>
          </NForm>
